...
## GPT-SoVITS 尾音吞字 (Early Cut-off) 問題排除

> 社群常見稱呼：「尾音吞字」、「斷氣」、「Early Cut-off」
> 模型訓練成功（聲音像）→ 問題出在推理 (Inference) 階段

### 根本原因

AI 模仿參考音檔的「收尾習慣」。若參考音檔結尾是漸弱/氣音，AI 學到這個模式，導致每句話結尾自動降低音量。

### 解法一：換「斷尾乾淨」的參考音頻（最有效）

- **避免**：結尾放鬆、拖長音、音量漸弱（Fade out）的參考音檔
  - 壞例子：「今天的報告就到這邊...（氣音）」
- **選用**：結尾堅定、短促、有力的句子（四聲/重音結尾）
  - 好例子：「我們必須採取行動。」（「動」字收尾乾淨有力）
- 剪輯一段講話非常有力或被打斷的片段作為參考
- 結尾越乾淨，AI 的氣越足

### 解法二：防禦性標點 (Text Prompting)

GPT 模型對標點符號非常敏感，句號常被解讀為「降低音量」。

| 技巧 | 原文 | 修改 | 效果 |
|------|------|------|------|
| 句號→驚嘆號 | surgery. | surgery\! | 語氣上揚避免衰減 |
| 句號→逗號 | surgery. | surgery, | AI 覺得話還沒講完 |
| 加廢字再剪掉 | prostatectomy | prostatectomy end end end | 衰減放在廢字上，前段飽滿清晰 |

### 解法三：調整 GPT 推理參數

| 參數 | 調整方向 | 說明 |
|------|---------|------|
| Top_P | 調高 (0.8 → 0.9~1.0) | AI 選字更大膽，不會保守收尾 |
| Temperature | 調低 (1.0 → 0.8) | 語氣更穩定，減少隨機斷氣 |

### 解法四：犧牲句法 (Sacrifice Sentence)

在合成文字最後加一句不需要的句子，生成後裁切掉。
已在 `gpt_sovits_health_edu_v2.wav` 驗證此方法有效。

### 建議優先順序

```
解法一（換參考音檔）→ 解法二（加廢字）→ 解法三（調參數）→ 解法四（犧牲句）
```

通常換一個「講話像是在命令或有力結尾」的參考音頻，問題就會瞬間消失。
